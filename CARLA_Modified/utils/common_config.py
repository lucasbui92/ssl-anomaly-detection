...
# Changes: Added if-else condition for Smart Grid dataset (2 types)
...
def get_train_dataset(p, transform, sanomaly, to_augmented_dataset=False,
    ...
    # Base dataset
    mean, std = 0, 0
    if p['train_db_name'] == 'smart_grid_minor':
        from data.SmartGrid import SmartGrid
        dataset = SmartGrid(p['fname'], train=True, transform=transform, sanomaly=sanomaly,
                      mean_data=None, std_data=None)
        mean, std = dataset.get_info()

    elif p['train_db_name'] == 'smart_grid_major':
        from data.SmartGrid import SmartGrid
        dataset = SmartGrid(p['fname'], train=True, transform=transform, sanomaly=sanomaly,
                      mean_data=None, std_data=None, dual_binary=True)
        mean, std = dataset.get_info()
...
def get_val_dataset(p, transform=None, sanomaly=None, to_neighbors_dataset=False,
    ...
    # Base dataset    
    if p['val_db_name'] == 'smart_grid_minor':
        from data.SmartGrid import SmartGrid
        dataset = SmartGrid(p['fname'], train=False, transform=transform, sanomaly=sanomaly,
