...
def load_model(model, fn, device = torch.device('cuda')):
    ...
    Args:
        fn (str): filename.
    '''
    state_dict = torch.load(fn, map_location=device, weights_only=False)
...
def find_period(time_series, id):
    ...
    # Real-FFT (one-sided) and frequencies (Hz if d=1 sample units)
    spectrum = np.fft.rfft(time_series)
    freqs    = np.fft.rfftfreq(n, d=1.0)         # shape ~ (n//2 + 1,)
    psd      = np.abs(spectrum) ** 2

    # Exclude DC explicitly (index 0)
    if freqs.size <= 1:
        top3_period = np.array([n], int)        # no usable positive freqs
    else:
        psd_pos   = psd[1:]                     # drop DC
        freqs_pos = freqs[1:]
        # Top-3 dominant positive frequencies (handle short arrays)
        k = min(3, psd_pos.size)
        top_idx = np.argsort(psd_pos)[::-1][:k]
        dom_freqs = freqs_pos[top_idx]

        # Safe reciprocal: skip zeros just in case
        dom_freqs = dom_freqs[dom_freqs != 0]
        if dom_freqs.size > 0:
            top3_period = (1.0 / dom_freqs).round().astype(int)
        else:
            top3_period = np.array([n], int)

    long_period_list = [str(number) for number in list(range(213, 222))] + ['239','240','241']
    period_opt = np.sort(top3_period)
...
def sliding_window_mv(time_series, window_size, stride=1):
    """
    Multivariate sliding window.

    time_series: np.ndarray of shape (T,) or (T, C)
    returns:
        (B, W)  if input is 1D
        (B, W, C) if input is 2D
    """
    time_series = np.asarray(time_series)

    # 1D case: fallback to original behaviour
    if time_series.ndim == 1:
        return sliding_window(time_series, window_size, stride)

    # 2D case: multivariate
    n, C = time_series.shape
    num_slices = (n - window_size) // stride + 1
    slices = np.zeros((num_slices, window_size, C), dtype=time_series.dtype)

    for i in range(num_slices):
        start = i * stride
        end = start + window_size
        slices[i] = time_series[start:end, :]   # (W, C)
